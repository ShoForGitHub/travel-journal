    ...
    <div className="spot-card">
      ...
      <div className="description">
        ...
        <div className="location-info">
          ...
          <a href={props.googleMapsUrl} target="_blank"  rel="noopener noreferrer" className="map-link">View on Google Maps</a>
        </div>

        <h2 className="spot-name">{props.title}</h2>
        <h3 className="travel-date">{props.startDate} - {props.endDate}</h3>
        <p className="spot-intro">{props.introduction}</p>
      </div>
    </div>
  )
}